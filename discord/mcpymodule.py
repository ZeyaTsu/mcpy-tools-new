...
def convert_chunks_blocks(block=None, chunk=None):
    if block != None and chunk == None:
        res = block / 16
        msg_bot = f"{block} blocks equal to {res} chunks."
        return msg_bot 
    elif block == None and chunk != None:
        resp = chunk * 16
        msg_bot = f"{chunk} chunks equal to {resp} blocks."
        return msg_bot

def mineshaft_mirror(x_coords, z_coords):
    x_coords = x_coords * -1
    z_coords = z_coords * -1
    msg_bot = f"The second mineshaft is at X: {x_coords} Z: {z_coords}"
    return msg_bot

def perimeter(x_coords, y_coords, z_coords, second_x_coords, second_y_coords, second_z_coords): 
...
def volume(x_coords, y_coords, z_coords, second_x_coords, second_y_coords, second_z_coords): 
    x_fi = x_coords - second_x_coords
    if x_fi < 0:
        x_fi *= -1
    x_fi += 1
