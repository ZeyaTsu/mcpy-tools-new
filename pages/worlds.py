...
def playerdata_onread():
    global PLAYERDATA_PATH
    if PLAYERDATA_PATH == None:
        print(instructions_color + "Example. C:\\example\\roaming\\.minecraft\\saves\\myworld\\playerdata" + default)
        PLAYERDATA_PATH = str(input("Playerdata's directory (playerdata.dat) > "))
        FILENAME = str(input("File name (no need to precise .dat) > ")) 
        PLAYERDATA_PATH = PLAYERDATA_PATH + "\\" + FILENAME + ".dat"

    nbtfile = nbt.NBTFile(PLAYERDATA_PATH, 'rb')

    for tag in nbtfile.tags:
        print(instructions_color, tag.tag_info(), default)
    playerdata_inventory_onread()

def playerdata_inventory_onread():
    global PLAYERDATA_PATH

    nbtfile = nbt.NBTFile(PLAYERDATA_PATH, 'rb')
    inventory = nbtfile["Inventory"]
    print(f"{Fore.CYAN}Player's Inventory:", default)
    for item in inventory:
        if 'tag' in item.keys():
            item_tag = item['tag']
            if 'display' in item_tag.keys():
                display_tag = item_tag['display']
                if 'Name' in display_tag.keys():
                    print(f" - {display_tag['Name']} : {item['Count']}")
                else:
    ...
    print("write q! to go to the list (main).")
